classdef ExampleHelperMCLVisualization < handle
    ...
    methods
        function obj = ExampleHelperMCLVisualization(og)
        ...
        function plotStep(obj, amcl, estimatedPose, ranges, angles, numUpdates)
            ...
            [laserX, laserY] = pol2cart(angles + amcl.SensorModel.SensorPose(3) + estimatedPose(3), ranges);
            laserX = laserX+estimatedPose(1)+amcl.SensorModel.SensorPose(1);
            laserY = laserY+estimatedPose(2)+amcl.SensorModel.SensorPose(2);
            if ishandle(obj.Axes)
                if isempty(obj.ParticlePlot)
                    ...
                    % Plot laser scans
                    obj.LaserPlot = plot(obj.Axes, laserX, laserY, 'r.');
                else
                    ...
                    set(obj.LaserPlot, 'XData', laserX, 'YData', laserY);
                end
